...
async def main(room_url: str, token: str = None):
    async with aiohttp.ClientSession() as session:
        # Configure Daily.co transport for real-time communication
        transport = DailyTransport(
            room_url,
        ...
        # Configure speech-to-text (STT) and text-to-speech (TTS) services
        stt = DeepgramSTTService(name="STT", url='ws://127.0.0.1:8082/v1/listen')
        tts = ClearableDeepgramTTSService(name="Voice", aiohttp_session=session)

        # Configure the language model service
        llm = OpenAILLMService(
            name="LLM",
            api_key=get_secret("HF_TOKEN"),
            model="meta-llama/Meta-Llama-3-8B-Instruct",
        ...
        # Create a pipeline with the transport, STT, LLM, and TTS
